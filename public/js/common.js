...
logoutButton.addEventListener('click', async function() {
    try {
        const response = await fetch(`http://${process.env.ADDRESS}/user/logout`, {
            method: 'POST',
            headers: {
...
async function load() {
    try {
        ...
        document.getElementById('profile_image').src = data.profileImage ? `http://${process.env.ADDRESS}${data.profileImage}` : '/images/profile_img.png';
    } catch (error) {
